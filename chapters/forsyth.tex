...
\author{Richard Forsyth\affiliation{%intentionally left blank
}\orcid{}}
...
To illustrate this approach, a small test corpus, consisting of seven subcorpora, has been compiled, briefly described in \tabref{tab:forsyth:1}.
...
\end{quote}
as well as shorter subsequences, probably going down to 2-grams such as "a better", "better future" and "hardworking families". Suppose further that the program is processing the sentence shown in \tabref{tab:forsyth:4} (tabulated vertically, for convenience).
...
The beginning of the main output file resulting from processing by formulex of a training sample of 1117 texts from the seven text categories described in \tabref{tab:forsyth:1} is shown below. In this case the formulexicon generated by outgrams contained the most frequent 3- to 6-grams from each category.
...
The derived ordering is written to a text file for inspection and also, more usefully, to a data file to be processed in the R package so that it can be displayed visually. \figref{fig:forsyth:4} shows the results of this procedure for the 36 most frequent collocades from the medical leaflet category.
...
\todo[inline]{Better move this somewhere else, e.g. in the Introduction}
